...
import org.lhyf.config.model.Vo.SimplePubNamespaceVo;
...
public class PubPropertiesController {
    ...
    public RestResponseBo updatePubItem(Integer id,String property,String value,String intro){
        ...
        List<PubItem> list = pubItemService.getPubItemsByProperty(property);
        if(list!=null&& list.size() > 0){
            // 防止因为存在多个重复 property ,所以只能都比较一遍id是否相等
            for(PubItem item:list){
                if(item.getId() != id){
    ...
    public RestResponseBo savePubItem(String property,String value,String intro,Integer namespaceId){
        ...
        List<PubItem> list = pubItemService.selectItemByNSIdAndProperty(namespaceId,property);
        if(list!=null && list.size() > 0){
    ...
    public RestResponseBo<List<PubItem>> getPropertiesByNameSpaceId(Integer id){
        List<PubItem> items = pubItemService.findIteamByNameSpaceId(id);
    ...
    public RestResponseBo savePubNameSpace(String code, String name, String intro) {
        ...
        // 1. 查看code是否重复
        List<PubNameSpace> list = pubNamespaceService.getPubNameSpaceByCode(code);
        if(list != null && list.size() > 0){
    ...
    public RestResponseBo<List<SimplePubNamespaceVo>> getAllPubNameSpace(){
        ...
        List<PubNameSpace> namespaces = pubNamespaceService.findAll();

        SimplePubNamespaceVo vo;
        for(PubNameSpace namespace:namespaces){
    ...
    public RestResponseBo<PubNameSpace> getPubNameSpaceById(Integer id){
        PubNameSpace ns = pubNamespaceService.getPubNameSpaceById(id);
